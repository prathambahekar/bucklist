...
export function computeCollectionStats(
  ...
  const displayTotalCount = Math.max(totalMovies, knownCount || totalMovies);
  const watchedMovies = sortedMovies.filter((m) => m.watched);
  const watchedMoviesCount = watchedMovies.length;
  const progressPercent = displayTotalCount > 0 ? Math.round((watchedMoviesCount / displayTotalCount) * 100) : 0;
